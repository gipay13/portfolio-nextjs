...
import { IconBrandGithub, IconCode, IconHome, IconMoon, IconSun, IconUser } from "@tabler/icons-react";
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-3">
                <a href="https://github.com/gipay13" target="_blank" className="p-2 bg-slate-950 dark:bg-white rounded-full">
                    <IconBrandGithub className="size-5 dark:text-slate-950 text-white"/>
                </a>
                <button onClick={toggleDarkMode} className="p-2 bg-slate-950 dark:bg-white rounded-full cursor-pointer">
                    {isDark ? <IconSun className="size-5 text-slate-950"/> : <IconMoon className="size-5 text-white" />}
                </button>
            </div>
        </motion.nav>
    );
